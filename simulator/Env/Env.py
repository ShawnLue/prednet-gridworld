...
from simulator.agent.base import ACTION_LIST
from simulator.agent.agent import Agent
from simulator.agent.checkpoint import Checkpoint
from simulator.agent.obstacle import Obstacle
from simulator.utils.utils import act_cost
...
OB_SCALE_SIZE_LIST = [21, 25, 31, 35]
...
def buffer_filter(buff):
    ...
    '''
    buffer[abs(buffer - 64) < 30] = 64  # Goal
    buffer[abs(buffer - 128) < 30] = 128  # Obstacle
    '''
    return buff
...
class Env:
    ...
    def __init__(self, map_size=48, snapshot=False, collision_quit=False):
        '''
        map_size:
        snapshot: (unresolved)
        collision_quit:
    ...
    @classmethod
    def random_scene(cls, map_size, input_data, difficulty=None, seed=None):
        ...
        for i in range(ob_num):
            while True:
                i_size = np.random.choice(OB_SCALE_SIZE_LIST)
    ...
    def step(self, cmds):
        ...
        if self.agent_num != 0:
            ...
            if info['chCrash'] or (self.collision_quit and (
                        len(info['obCrash']) + len(info['agCrash']) + len(info['wallCrash'])) > 0):
                self.done = True
